...
- On dit que $f$ est dérivable en $a$ (resp. sur $I$) lorsque $f^{(k)}(a)$ existe (resp. $f^{(k)}$ est définie sur $I$). 
...
- $x \mapsto \sqrt{x}$ est infiniment continue sur $\mathbb{R}_+^*$ mais seulement $C^0$ en $0$. 
...
- $g: x \mapsto \begin{cases} x^2 \sin(1/x) & \text{si} & x \neq 0 \\ 0 & \text{si} & x = 0 \end{cases}$ 
...
Trouver les minimums et maximums globaux des fonctions suivantes:
$$
\begin{align*}
...
f(x) = 3\cos(2x) - 5x & & \text{ pour } x \in [0, 6] \\
...
$$
\begin{align*}
\forall x \in \mathbb{R} \quad e^x \geq 1 + x & & \forall x \geq 0 \quad xe^x + 1 \geq e^x \geq 1 + x + \frac{x^2}{2} \\
\forall x \in (-1, +\infty) \quad \ln(1+x) \leq x & & \forall x \leq 0 \quad 1+x \leq e^x \leq 1 + x + \frac{x^2}{2}
\end{align*}
$$
