...
Quelle est la vitesse de dégradation de chaque bactérie, par unité de $10^8$ UFC / g? On suppose que la vitesse de dégradation est additive. 
...
\begin{enumerate}
\item Montrer par récurrence que pour tout entier $n \geq 0$, il existe deux réels $a_n$ et $b_n$ tels que $f^{(n)}(x) = (a_n x + b_n)e^{-x}$. On pourra expliciter la relation de récurrence entre $(a{n+1}, b_{n+1})$ et $(a_n, b_n)$. 
\item Expliciter $a_n$. 
...
où $c$ est la vitesse de propagation du son dans le milieu ambiant, $v_e$ est la vitesse de déplacement de la source vers l'auditeur (positive si la source se rapproche, négative sinon) et $v_r$ est la vitesse de déplacement de l'auditeur de l'auditeur vers la source (positive si la source se rapproche, négative sinon). 
...
\begin{itemize}
\item Montrer que pour la fraction $1/(1 - y^2)$ peut se décomposer sous la forme 
\begin{equation*}
...
Sakura (\begin{CJK}{UTF8}{min}桜\end{CJK}) cherche à attraper $N$ cartes de Clow (\begin{CJK}{UTF8}{min}クロウカード\end{CJK}). Il faut un temps $T/N$ pour attraper la première carte, $T/(N-1)$ pour la deuxième, $T/(N-2)$ pour la troisième et ainsi de suite. 
